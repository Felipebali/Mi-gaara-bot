const handler = async (m, { conn, text, usedPrefix, command, args, isROwner }) => {
    ...
    const suittag = '59898719147' // número del dueño
    ...
    try {
        ...
        let who = mentionedJid[0] ? mentionedJid[0] : m.quoted ? m.quoted.sender : text ? no(text.split(' ')[0]) + '@s.whatsapp.net' : false
        ...
        switch (command) {
            ...
            case 'unban': {
                ...
                if (!users[who].banned) return conn.reply(m.chat, `ꕥ @${who.split('@')[0]} no está baneado.`, m, { mentions: [who] })
                ...
                await conn.reply(`${suittag}@s.whatsapp.net`, `❀ ${nameUnban} fue desbaneado por ${await conn.getName(m.sender)}.`)
                break
            }
